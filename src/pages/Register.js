...
function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!authData.user) {
                throw new Error('Erro: Dados do usuário não foram criados corretamente');
            }
            ...
            const userData = {
                ...
                cep: currentForm.cep.replace(/\D/g, ''),
                numero: currentForm.numero,
                complemento: currentForm.complemento || null,
                auth_id: authData.user.id
            };

            // Adicionar CNPJ ou CPF dependendo do tipo
            if (formType === 'pessoaFisica') {
                userData.cpf = currentForm.cpf.replace(/\D/g, '');
            } else {
                userData.cnpj = currentForm.cnpj.replace(/\D/g, '');
            }

            console.log('Tentando inserir:', userData);
            ...
            if (userError) {
                console.error('Erro detalhado na inserção:', userError);
                throw new Error('Erro ao salvar dados do usuário: ' + userError.message);
            }

            alert('Cadastro realizado com sucesso! Por favor, verifique seu e-mail para confirmar o cadastro antes de fazer login.');
            navigate('/login');
